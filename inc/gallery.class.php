...
class flgalleryGallery extends flgalleryBaseClass
{
			...
			if ($images !== false)
			{
				...
				foreach ($images as $img)
				{
					...
					if (
						...
						$img->link = "javascript:jQuery.altbox('{$img->source}');";
						$img->target = "_self";
					}
	...
	function getThumbSize()
	{
		switch ($this->type)
		{
			...
			case 'PhotoFlow':
				$width = (int)$this->settings['maxImageWidth'];
				$height = 0;
				break;
			...
			default:
				$width = isset($this->settings['thumbnail.width']) ? (int)$this->settings['thumbnail.width'] : 0;
				$height = isset($this->settings['thumbnail.height']) ? (int)$this->settings['thumbnail.height'] : 0;
				break;
		}
