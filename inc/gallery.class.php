...
class flgalleryGallery extends flgalleryBaseClass
{
	...
	function get_settingsXml()
	{
		include FLGALLERY_GLOBALS;

		if ( empty($plugin->galleryInfo[$this->type]['settings']) )
		{
			$settingsDir = $this->is_legacy() ? 'settings.legacy' : 'settings';
	...
	function get_settings($reload = false)
	{
		...
		if ( $reload || empty($this->settings) )
		{
			...
			if ( $results !== false )
			{
				...
				$settingsXml = $this->get_settingsXml();
	...
	function get_html()
	{
		...
		$this->get_settings();
		if ($this->settings['background.transparent'] == 'false') {
			$background = '';
	...
	function get_xml()
	{
		if ($this->id)
		{
			...
			$xmlDir = $this->is_legacy() ? 'xml.legacy' : 'xml';
			...
			if ($images !== false)
			{
				...
				foreach ($images as $img)
				{
					...
					$image = new flgalleryImage($img->id);
					$image->load();
					...
					if ($size['width'] < $img->width || $size['height'] < $img->height)
						$img->source = $image->resized($size, true);

					if ($thumbSize['width'] < $img->width && $thumbSize['height'] < $img->height)
					{
						$thumbnail = $image->resized($thumbSize, true);
	...
	function get_thumbSize()
	{
		...
		{
			case 'Art':
				if ($this->settings['preview.userPreview'] == 'true') {
					$width = (int)$this->settings['preview.width'];
					$height = (int)$this->settings['preview.height'];
				}
				else {
					$width = (int)$this->settings['thumbnail.width'];
					$height = (int)$this->settings['thumbnail.height'];
				}
				break;

			case 'StackPhoto':
				$width = (int)$this->settings['image.width'];
				$height = (int)$this->settings['image.height'];
				break;

			case 'Zen':
				$width = (int)$this->settings['iconWidth'];
				$height = (int)$this->settings['iconHeight'];
				break;
