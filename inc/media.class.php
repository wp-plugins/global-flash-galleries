...
class flgalleryMedia extends flgalleryBaseClass
{
		...
		switch ($action)
		{
			...
			case 'updateAlbum':
				if ( !empty($_REQUEST['update']) )
				{
					if ( !empty($album_id) )
					{
						$wpdb->update( $plugin->dbAlbums, $_REQUEST['album'], array('id' => $album_id) );
					}
			...
			case 'uploadPage':
				$media->uploadPage( array('album_id'=>$album_id) );
				break;
			...
			case 'upload':
				if ( !empty($_REQUEST['OK']) )
				{
					if ( !empty($album_id) )
					{
						$media->uploadPictures( array('album_id' => $album_id) );
						$func->locationReset('&action=editAlbum&album_id='.$album_id);
						$media->editAlbum($album_id);
					}
				}
	...
	function albums()
	{
		...
		if ( $albums !== false && count($albums) )
		{
			...
			foreach ($albums as $album)
			{
				...
				$album->upload = $admpage->actionButton( 'Upload Pictures', 'uploadPage', array('album_id'=>$album->id) );
	...
	function editAlbum($album_id)
	{
		...
		if ($album !== false)
		{
			...
			$album->href = $admpage->href;
			$album->upload = $admpage->actionButton( array('+ Upload', true), 'uploadPage', array('album_id'=>$album_id) );
	...
	function uploadPage($a)
	{
		...
		$admpage->head('Upload Pictures', 'upload');
		...
		$admpage->tabmenu(
			...
			$tab, '&amp;action=uploadPage'.$objectID
		);

		switch ($tab)
		{
			case 'standard':
				$uploadPage = $media->standardUploader($a, false);
				break;
			case 'url':
				$uploadPage = $media->addFromURL($a, false);
				break;
			default:
				$uploadPage = '';
		}

		$tpl->t( 'media/upload', array(
	...
	function standardUploader(&$a, $echo = true)
	{
		...
		$out = $tpl->parse('media/upload-standard', $a);
	...
	function addFromURL(&$a, $echo = true)
	{
		...
		$out = $tpl->parse('media/upload-url', $a);
	...
	function uploadPictures($a)
	{
		...
		$uploaded = $media->uploadFiles($plugin->imgDir);
		if ( count($uploaded) )
			...
			if ( !empty($path) )
			{
				...
				$insert = $wpdb->insert(
					...
					array(
						...
						'type' =>		$imageSize['mime'],
						'path' =>		$path,
						'name' =>		basename($_POST['addfromurl_file'][$key]) || $_FILES['stdupload_file']['name'][$key],
	...
	function uploadFiles($destDir)
	{
		...
		if ( !empty($_POST['addfromurl_file']) && is_array($_POST['addfromurl_file']) )
		{
			...
			return $func->copyURL($URLs, $destDir, $destNames);
		}
	}
