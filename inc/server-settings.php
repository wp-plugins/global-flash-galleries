...
function bytesToM($size, $units = 'M', $precision = 1)
{
	$k = array(
		'' => 1,
		'B' => 1,
	...
	return round( $size / $k[strtoupper($units[0])], $precision );
}
endif;
...
elseif ( version_compare(PHP_VERSION, '5.2.6', '<') || version_compare(PHP_VERSION, '5.3.0', '>=') )
...
else $allow_url_fopen = '<strong class="red">'.__('Off', FLGALLERY_NAME).'</strong>';
...
if(ini_get('memory_limit'))
{
	...
	$memory_limit = sprintf($tMB, bytesToM($mem_limit));
}
else
...
if (function_exists('memory_get_usage'))
{
	...
	if ($mem_limit - $mem < mToBytes('8M'))
		$memory_usage = sprintf('<strong class="red">'.$tMB.'</strong>', bytesToM($mem));
	else
		$memory_usage = sprintf($tMB, bytesToM($mem));
}
else
