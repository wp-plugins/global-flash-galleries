...
class flgalleryFunctions extends flgalleryBaseClass
{
	...
	function init()
	{
		$this->preg_pluginDir = preg_quote(FLGALLERY_PLUGIN_DIR, '/');
		$this->preg_siteDir = preg_quote(FLGALLERY_SITE_DIR, '/');
	}

	function url($path, $plugin = false)
	{
		if ($plugin)
			$path = preg_replace('/^'.$this->preg_pluginDir.'[\\/\\\\]*(.*)$/', FLGALLERY_PLUGIN_URL.'/$1', $path);
		else
			$path = preg_replace('/^'.$this->preg_siteDir.'[\\/\\\\]*(.*)$/', FLGALLERY_SITE_URL.'/$1', $path);
			...
			else	
			{
				foreach ( $f['name'] as $key => $name )
				{
					if ( $f['error'][$key] == UPLOAD_ERR_OK && is_uploaded_file($f['tmp_name'][$key]) )
					{
						...
						{
							@chmod($destDir.'/'.$destName[$key], 0777);
	...
	function short_filename( $filename, $maxLength = 20, $dots = '..' )
	{
