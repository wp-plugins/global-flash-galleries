...
class flgalleryTemplates extends flgalleryBaseClass
{
	...
	function _fast($text, &$a, &$tags)
	{
		$qtags = array( preg_quote($tags[0]), preg_quote($tags[1]) );

		
		if ( preg_match_all('#'.$qtags[0].'(TRUE|FALSE)\s+?(.*?)\s*?'.$qtags[1].'(.*?)'.$qtags[0].'/(TRUE|FALSE)'.$qtags[1].'#ms', $text, $m) )
		{
		...
		if ( preg_match_all('#'.$qtags[0].'(IF)\s+?(.*?)\s*?'.$qtags[1].'(.*?)'.$qtags[0].'/(IF)'.$qtags[1].'#ms', $text, $m) )
		{
			foreach ($m[0] as $i => $val)
			{
				if ( $m[1][$i] == $m[4][$i] )
		...
		if ( preg_match_all('#'.$qtags[0].'@(.*?)'.$qtags[1].'#', $text, $m) )
		{
		...
		if ( preg_match_all('#'.$qtags[0].'([\w_]+[\w\d\._]*?)'.$qtags[1].'#', $text, $m) )
		{
			$keys = array_unique($m[1]);
			foreach ($keys as $key)
			{
