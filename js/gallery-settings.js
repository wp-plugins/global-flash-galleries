...
jQuery(document).ready(function($) {
	...
	/*
	// defaults
	$.colorInput.defaults.cells = 32;
	$.colorInput.defaults.cellSize = 3;
	$.colorInput.defaults.hueWidth = 20;
	...
	var $input = $("#flashSettings input.color").colorInput({
		colorSelected: function(e) {
			this.value = this.value.match(/[0-9A-F-a-f]+/);
		}
	});
	...
	$(document).click(function(e) {
		if ($.colorInput.current) {
			if ($.colorInput.current.isChanged && window.confirm("Accept selected colour change?")) {
